...
public class BaseResourceTest extends JerseyTest {
  ...
  protected static final Credentials ADMIN_CREDENTIALS =
      new Credentials("Admin", "password", Role.ADMIN);
  protected static final User TEST = new User(2, Role.USER, "Test");
  protected static final Credentials TEST_CREDENTIALS =
      new Credentials("Test", "password", Role.USER);
  ...
  protected void login(Credentials credentials) {
    authedSession =
        new UserDataAccess(Config.instance().getDatabaseDriver()).authenticate(credentials);
  }

  protected void logout() {
    new UserDataAccess(Config.instance().getDatabaseDriver())
        .removeSession(authedSession.getSessionId());
    authedSession = null;
  }
