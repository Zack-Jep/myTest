...
public class UserResourceTest extends BaseResourceTest {
  ...
  public void loginRememberMeCookie() {
    ...
    Response loginWithoutRememberMe =
        target("user").path("login").request().post(Entity.json(TEST_CREDENTIALS));
  ...
  public void logout() {
    ...
    Response loginResponse =
        target("user").path("login").request().post(Entity.json(ADMIN_CREDENTIALS));
  ...
  public void deleteUserAsUser() {
    ...
    target("user")
        ...
        .request()
        .delete(Void.class); // Include response type to
    // trigger exception
  }
  ...
  // Credentials need to be updated to accept email and have Role optional.
  @Test
  public void testRegisterUser() {
    Credentials newCredentials = new Credentials("pelle", "passphrase", Role.USER);
    User newUser =
        target("user").path("register").request().post(Entity.json(newCredentials), User.class);
