...
/**
 * Contains helpers for creating the database schema. Each time the server starts the @{@link
 * #createSchemaIfNotExists()} method is called.
 *
...
public class CreateSchema {
  ...
  private static void runScript(Connection conn) throws SQLException {
    RunScript.execute(
        conn, new InputStreamReader(DataAccess.class.getResourceAsStream("schema.sql")));
  }
}
