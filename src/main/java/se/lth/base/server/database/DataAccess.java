...
/**
 * Base class for H2 database connections. Contains helper methods for common JDBC use cases Also
 * contains a method for setting up the database schema. To use this class, extend this class with a
 * target data type parameter and provide a mapper for said type.
 *
...
public class DataAccess<T> {
  ...
  public T queryFirst(String sql, Object... objects) {
    return queryStream(sql, objects)
        .findFirst()
        .orElseThrow(() -> new DataAccessException(ErrorType.NOT_FOUND));
  }
    ...
    switch (h2Code) {
      ...
      case ErrorCode.CHECK_CONSTRAINT_VIOLATED_1:
        return new DataAccessException(
            "Resource constraints violated", cause, ErrorType.DATA_QUALITY);
      default:
