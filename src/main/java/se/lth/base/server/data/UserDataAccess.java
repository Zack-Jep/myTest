...
/**
 * Basic functionality to support standard user operations. Some notable omissions are removing
 * user, time out on sessions, getting a user by name or id, etc.
 *
 * <p>This is intended to be as realistic as possible with reasonable security (single factor
 * authentication). The security measures are as follows.
 *
 * <ul>
 *   <li>All passwords are stored in a hashed format in the database, using @{@link
 *       Credentials#generatePasswordHash(long)}}
 *   <li>Usernames are used to salt passwords, <a
 *       href="https://en.wikipedia.org/wiki/Salt_(cryptography)">see here for explanation.</a>
...
public class UserDataAccess extends DataAccess<User> {
  ...
  public User addUser(Credentials credentials) {
    ...
    int userId =
        insert(
            "INSERT INTO user (role_id, username, password_hash, salt) VALUES (("
                + "SELECT role_id FROM user_role WHERE user_role.role=?),?,?,?)",
            credentials.getRole().name(),
            credentials.getUsername(),
            credentials.generatePasswordHash(salt),
            salt);
  ...
  public User updateUser(int userId, Credentials credentials) {
    if (credentials.hasPassword()) {
      long salt = Credentials.generateSalt();
      execute(
          "UPDATE user SET username = ?, password_hash = ?, salt = ?, role_id = ("
      ...
      execute(
          "UPDATE user SET username = ?, role_id = ("
              + "    SELECT user_role.role_id FROM user_role WHERE user_role.role = ?) "
              + "WHERE user_id = ?",
          credentials.getUsername(),
          credentials.getRole().name(),
  ...
  public User getUser(int userId) {
    return queryFirst(
        "SELECT user_id, role, username FROM user, user_role "
            + "WHERE user.user_id = ? AND user.role_id = user_role.role_id",
        userId);
  }
  ...
  public List<User> getUsers() {
    return query(
        "SELECT user_id, username, role FROM user, user_role "
  ...
  public Session getSession(UUID sessionId) {
    User user =
        queryFirst(
            "SELECT user.user_id, username, role FROM user, user_role, session "
                + "WHERE user_role.role_id = user.role_id "
                + "    AND session.user_id = user.user_id "
                + "    AND session.session_uuid = ?",
            sessionId);
    execute(
        "UPDATE session SET last_seen = CURRENT_TIMESTAMP() " + "WHERE session_uuid = ?",
        sessionId);
  ...
  public Session authenticate(Credentials credentials) {
    ...
    long salt =
        new DataAccess<>(getDriverUrl(), (rs) -> rs.getLong(1))
    ...
    UUID sessionId =
        insert(
            "INSERT INTO session (user_id) " + "SELECT user_id from USER WHERE username = ?",
            user.getName());
    return new Session(sessionId, user);
  }
}
