...
/**
 * FooDataAccess is a place-holder that shows how the base system can be extended with additional
 * functionality.
 *
 * <p>Note that the name 'FooDataAccess' is not very good, since it is generic. In your project you
 * are expected to give more specific names to your classes, see for example @{@link UserDataAccess}
 * which is more aptly named.
 *
...
public class FooDataAccess extends DataAccess<Foo> {
  ...
  public Foo addFoo(int userId, String payload) {
    ...
    int fooId =
        insert(
            "INSERT INTO foo (user_id, payload, created) VALUES (?,?,?)",
            userId,
            payload,
            new Date(created));
