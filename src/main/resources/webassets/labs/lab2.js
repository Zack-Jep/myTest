...
var sqlBaseValidate = function(submitEvent) {
  submitEvent.preventDefault();
  var checked = Array.from(document.querySelectorAll('#sqlBase input')).map(
    i => i.checked
  );
  baseLab.complete(
    'sqlBase',
...
var sqlSchemaValidate = function(submitEvent) {
  submitEvent.preventDefault();
  var checked = Array.from(document.querySelectorAll('#sqlSchema input')).map(
    i => i.checked
  );
  baseLab.complete(
    'sqlSchema',
    ...
    .then(function() {
      return fetch('/rest/foo', { credentials: 'same-origin' })
        ...
        .then(function(foos) {
          ...
          if (match.total !== 5) {
            throw 'Got wrong total when testing implementation: expected 5 but received ' +
              f.total;
          }
        });
    })
