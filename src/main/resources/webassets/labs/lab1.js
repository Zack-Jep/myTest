  ...
  base.rest
    ...
    .then(function(data) {
      if (data.error) {
        ...
        baseLab.complete('e2eServer', false);
        alert(
          'The "total" field has the wrong type. It should be number but got ' +
            typeof data[0].total
        );
      } else {
...
var e2eTableValidate = function() {
  fetch('/foo/foo.html')
    ...
    .then(function(html) {
      ...
      var tdsOk =
        d.querySelector('#foo-template').content.querySelectorAll('td')
          .length == 3;
