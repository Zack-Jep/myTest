...
var baseLab = {
  init: function(answerSheet, previousUrl, nextUrl) {
    ...
    fetch('header.html')
      ...
      .then(function(headerHTML) {
        ...
        document
          .querySelectorAll('a.anchor:not([name="Top"])')
          .forEach(function(mainAnchor, i) {
            function fixLink(prefix, anchor, hx, ix) {
              ...
              var heading =
                ...
                (ix + 1) +
                '. ' +
                section.querySelector(hx).textContent;
              a.href = '#' + name;
              ...
              var nextLabRef = t.parentElement.querySelector(
                'div.dropdown-divider:last-of-type'
              );
  ...
  scrollToNext: function() {
    ...
    if (!next) return;
    var anchor =
      next.parentElement.querySelector('a.sub-anchor') ||
      next.parentElement.querySelector('a.anchor');
    if (anchor) {
