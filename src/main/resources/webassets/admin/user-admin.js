...
base.userAdminController = function() {
  ...
  var view = {
    ...
    removeUser: function(user, ix) {
      var el = document.getElementById('user-list').querySelectorAll('button')[
        ix
      ];
    ...
    selectUser: function(user, clickedEl) {
      ...
      document.getElementById('user-data').querySelector('a').href =
        '/rest/foo/user/' + user.id;
  ...
  var controller = {
        ...
        base.rest.addUser(credentials).then(function(user) {
          if (user.error) {
            ...
            var el = document.querySelector(
              '#user-list .list-group button:last-of-type'
            );
            el.onclick = () => view.selectUser(user, el);
            view.selectUser(user, el);
          }
    ...
    load: function() {
      document.getElementById('change-password').onclick = event =>
        view.editPassword(false);
      ...
      Promise.all([
        ...
        userEls.forEach(function(el, i) {
          if (i == userEls.length - 1) {
            el.onclick = () =>
              view.selectUser(
                { username: '', role: model.roles[0], id: '' },
                el
              );
          } else {
